...
VIMPORT IVisPlugin_cl* GetEnginePlugin_rmuxPlugin();
VIMPORT IVisPlugin_cl* GetEnginePlugin_vScaleformPlugin();

class rmuxApplicationClass : public VAppImpl, public IVisCallbackHandler_cl
{
public:
  rmuxApplicationClass() 
	  : m_sMovieName("")
	  , m_spMovie(NULL)
	  , m_spMouse(NULL)
  {}

  virtual ~rmuxApplicationClass() {}
...
VAPP_IMPLEMENT_SAMPLE(rmuxApplicationClass);

void rmuxApplicationClass::SetupAppConfig(VisAppConfig_cl& config)
{
  ...
  config.m_videoConfig.m_iXPos = -660;   // Set the Window position X if not in fullscreen.
  ...
  config.m_videoConfig.m_szWindowTitle = "rmux";
...
void rmuxApplicationClass::PreloadPlugins()
{
  ...
  VISION_PLUGIN_ENSURE_LOADED(rmuxPlugin);
...
//---------------------------------------------------------------------------------------------------------
void rmuxApplicationClass::Init()
{
...
//---------------------------------------------------------------------------------------------------------
void rmuxApplicationClass::AfterSceneLoaded(bool bLoadingSuccessful)
{
  ...
  VString strScoreName = "Flash\\ingame.swf";
...
void rmuxApplicationClass::DeInit()
{
	// De-Initialization
	// [...]
	VOnExternalInterfaceCall::OnExternalInterfaceCallback -= this;
...
//---------------------------------------------------------------------------------------------------------
void rmuxApplicationClass::OnHandleCallback(IVisCallbackDataObject_cl *pData)
{
